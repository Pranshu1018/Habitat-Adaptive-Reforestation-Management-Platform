...
export interface SimulationScenario {
  type: 'drought' | 'flood' | 'heat' | 'species_failure' | 'custom';
  intensity: 'low' | 'medium' | 'high';
  duration: number; // days
  overrides?: {
    tempAdjustment?: number; // +/- degrees
    rainfallMultiplier?: number; // 0.5 = 50% rain
...
class DataIntegrationService {
  ...
  async analyzeSite(
    ...
    try {
      ...
      // Deep clone data to prevent cache mutation during simulation
      let weatherData = simulationMode ? JSON.parse(JSON.stringify(weather)) : weather;
      let soilData = simulationMode ? JSON.parse(JSON.stringify(soil)) : soil;
      let vegetationData = simulationMode ? JSON.parse(JSON.stringify(vegetation)) : vegetation;

      // Get weather forecast
      const forecast = await weatherService.getForecast(lat, lon);
      weatherData.forecast = simulationMode ? JSON.parse(JSON.stringify(forecast)) : forecast;

      // Apply simulation modifications if in simulation mode
      if (simulationMode && simulationScenario) {
        this.applySimulationScenario(weatherData, soilData, vegetationData, simulationScenario);
      }

      // Calculate suitabilityScore calculation using cloned data
      ...
      // Get species recommendations using cloned data
      ...
      // Assess risks using cloned data
      ...
      // Calculate carbon sequestration using cloned data
      ...
      // Get soil amendments
      const soilAmendments = await soilService.analyzeSoilAmendments(soilData);
    ...
    switch (scenario.type) {
      ...
      case 'custom':
        if (scenario.overrides) {
          const { tempAdjustment, rainfallMultiplier, humidityMultiplier } = scenario.overrides;

          if (tempAdjustment) {
            weather.current.temp += tempAdjustment;
            weather.forecast.forEach(d => d.temp += tempAdjustment);
          }
          if (rainfallMultiplier !== undefined) {
            weather.forecast.forEach(d => d.precipitation *= rainfallMultiplier);
          }
          if (humidityMultiplier !== undefined) {
            weather.forecast.forEach(d => d.humidity *= humidityMultiplier);
          }
        }
        break;
    }
